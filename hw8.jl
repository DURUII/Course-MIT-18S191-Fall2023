...
# ╔═╡ 46cb130c-836e-4b7d-b89f-4941a9144419
import PlotJS, GR
...
md"""
Before we jump in to simulating the SIR equations, let's experiment with a simple 1D function. In calculus, we learn techniques for differentiating and integrating _symbolic_ equations, e.g. ``\frac{d}{dx} x^n = nx^{n-1}``. 

But in real applications, it is often impossible to apply these techniques, either because the problem is too complicated to solve symbolically, or because our problem has no symbolic expression, like when working with experimental results.
...
function finite_difference_slope(f::Function, x, Δ = 1e-4)
...
function tangent_line(f, a, h)
	k = finite_difference_slope(f, a, h)
    return x -> 1.0 * k * (x - a) + f(a) 
end

# ╔═╡ 2b79b698-10b9-11eb-3bde-53fc1c48d5f7
# this is our test function
wavy(x) = 0.1x^3 - 1.6x^2 + 7x - 3;
...
@bind log_h Slider(-16:0.01:0.5, default = -0.5)

# ╔═╡ 7495af52-10ba-11eb-245f-a98781ba123c
h_finite_diff = 10.0^log_h

# ╔═╡ cef24060-ec6f-47c5-b8d0-60a802e1fbc1
md"""
✍️ if h is rendered too small, the tangent line will become a horizontal line.
"""
...
function euler_integrate(fprime::Function, fa::Number, T::AbstractRange)
	...
	for a in T
		push!(result, euler_integrate_step(fprime, result[end] , a, h))
	end
...
# ╔═╡ ab72fdbe-10be-11eb-3b33-eb4ab41730d6
 md" ``N_{euler} = `` $(@bind N_euler Slider(2:220, show_value=true))"
...
let
    ...
    try
        plot!(p, a .+ h .* (1:N_euler), history, color = "blue", label = nothing)
        scatter!(
            p,
            a .+ h .* (1:N_euler),
            history,
...
euler_SIR_step(0.1, 0.05, [0.99, 0.01, 0.00], 0.1)
...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	for x in T
		push!(result, euler_SIR_step(β, γ, result[end], h))
	end
...
# ╔═╡ 159f84e0-a1bd-40a4-9ae5-c2ac0e8db258
length(sir_results)
...
_in this case, there is convergence after a long time, so statistically speaking, after the dynamic balance, the the number of infected people remain next to 0, which means that not everybody need to get infected._
"""
...
md" ``β = `` $(@bind β Slider(0:0.1:1, default=0.5, show_value=true))"

# ╔═╡ 6f5ec14b-de21-450f-ad7b-0dcc85ae608b
md" ``γ = `` $(@bind γ Slider(0:0.01:1, default=0.15, show_value=true))"

# ╔═╡ fd180906-c947-47ff-9700-c20138af8bf8
let
	my_answer = md"""
	_given β, as γ increases, the peak of the infected would drop._
	
	_given γ, as β increases, the peak of the infected would strike and advance._
	"""
end

# ╔═╡ 86b6ea53-519d-478e-8515-3987412af74b
let 
	sir_T = 0:0.1:50.0
	sir_results = euler_SIR(β, γ, [0.99, 0.01, 0.00], sir_T)
	plot_sir!(plot(), sir_T, sir_results)
end
...
function ∂x(f::Function, x, y, Δ = 1e-4)
...
function ∂y(f::Function, x, y, Δ = 1e-4)
...
function gradient_1d_viz(N_gradient_1d, x0)
    ...
    history = accumulate(1:N_gradient_1d, init = x0) do old, _
        gradient_descent_1d_step(f, old, η = 0.025)
    end
    ...
    p = plot(x, f, label = "f(x)", lw = 3, opacity = 0.6, color = "gray")
    # p = plot()
    ...
    plot!(
        ...
        f,
        color = "blue",
        opacity = range(0.5, step = 0.2, length = length(all)),
        label = nothing,
    )
    scatter!(
        p,
...
1. _not improving in several rounds, the value of f does not descend any more_
2. _the gradient is approaching 0_
"""
...
function gradient_descent_2d(f, x0, y0; η = 1)
	boss_x, boss_y, boss_f = x0, y0, f(x0, y0)
	counter = 10
...
# ╔═╡ c05de701-9783-4235-99c4-48f93112a56e
...
# ╔═╡ b59e7cfc-c2ae-4bc4-b153-ad66acf34c38
...
function gradient_2d_viz_2d(N_gradient_2d, x0, y0)
    ...
    p = heatmap(-4:0.4:5, -4:0.4:4, himmelbau, c = :vikO100)
...
my_answer = md"""
_there should be 3 local minima/critical points._
"""
...
function dice_frequencies(N_dice, N_experiments)
    ...
    results = [experiment() for _ = 1:N_experiments]
...
👋reference: [Normal Distribution](https://www.lfhacks.com/t/dice-normal/#:~:text=当只有1颗骰子时，骰子的点数符合均匀分布%E3%80%82%20当扔出多次骰子时，每次骰子点数的总和，会随着试验次数的增多，接近正态分布%E3%80%82%20我们下面用一次投出%20N%20颗骰子代替1颗骰子投出%20N%20次%E3%80%82%20比如一次投5颗骰子，得到,sim%20N%20%28mu%2C%20sigma%29%20X%20∼%20N%20%28μ%2Cσ%29)
"""

# ╔═╡ 8e38298b-c51b-467b-9d2b-0009fe2025d8
let
	a=1:30
	a[end]
end
...
md"μ = $(@bind guess_μ Slider(1:0.1:last(dice_x); default = (dice_x[1] + dice_x[end]) / 2, show_value=true))"
...
function loss_dice(μ, σ)
	result = 0
	for (xi, yi) in zip(dice_x, dice_y)
		item = gauss(xi, μ, σ) - yi
		result += item * item
	end
    return result
end
...
# ╔═╡ 649e2788-c094-48f7-ade7-bc57c055b3e1
length(spatial_results)
...
function loss_sir(β, γ)
	...
	truth_sir = spatial_results
	guess_sir = euler_SIR(β, γ, spatial_results[1], spatial_T)
	
	ce = (y, p) -> −(y * log(p) + (1 − y) * log(1−p))
	
	for i in 1:length(truth_sir)
		result += sum(ce.(guess_sir[i].+1e-10, truth_sir[i].+1e-10))
	end
...
found_β, found_γ = let
    # your code here
	gradient_descent_2d(loss_sir, 0.02, 0.003, η=1e-8)
end

# ╔═╡ 496b8816-12d3-11eb-3cec-c777ba81eb60
let
    p = plot()
    plot_sir!(p, spatial_T, spatial_results, label = "spatial", opacity = 0.7)

    if show_manual_sir_fit
        guess_results = euler_SIR(guess_β, guess_γ, [0.99, 0.01, 0.00], spatial_T)
    ...
    try
        ...
        found_results = euler_SIR(found_β, found_γ, [0.99, 0.01, 0.00], spatial_T)
...
elseif !(euler_test isa Vector) || (abs(length(euler_test) - 101) >= 1)
    keep_working(
        md"Make sure that you return a vector of numbers, of the same size as `T`.",
    )
else
...
let
    ...
    elseif !(result isa Real)
        ...
        if error > 5.0
            almost(
                md"It's not accurate enough yet. Maybe you need to increase the number of steps?",
            )
        elseif error > 0.02
            keep_working()
        else
...
let
    if (found_β isa Missing) || (found_γ isa Missing)
        ...
        if isnan(found_β) || isnan(found_γ)
            hint(
                md"The found parameters are `NaN`, which means that floating point errors led to an invalid value. Try setting ``\eta`` much lower, try `1e-6`, `1e-7`, etc.",
            )
        else
            diffb = abs(found_β - 0.019)
            diffc = abs(found_γ - 0.0026)

            if diffb > 0.1 || diffc > 0.01
                almost(
                    md"Try using initial values that are closer to the expected result. (For example, the values that you found using the sliders.)",
                )
            elseif diffb > 0.01 || diffc > 0.001
                almost(
                    md"Try using initial values that are closer to the expected result. (For example, the values that you found using the sliders.)
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "fcba3cda8cd834034ca66496bfe9d706ebfdc49a"

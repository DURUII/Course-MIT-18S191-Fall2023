...
rand(10, 12) # An example matrix (two-dimensional array)
...
R2 = RankOneMatrix(rand(2), rand(3))
...
function matvec(M::RankOneMatrix, x)
	...
	return sum(M.w .* x) * M.v # Your code here
end
...
# ╔═╡ 862f230d-2c73-44da-a0d9-79f2fc00a359
...
function Base.getindex(M::LowRankMatrix, i, j)
	# sum(Ms)[i, j]
	return sum(sum([M.Ms[k][i, j] for k in 1:length(M.Ms)])) # Your code here
end
...
# ╔═╡ 6e35705b-70dd-44f2-a6a4-ea5522c799d3
begin
	R3a = RankOneMatrix([1,2], [3,4,5])
	R3b = RankOneMatrix([2,1], [3,4,5])
	R4 = LowRankMatrix([R3a, R3b],2)
end
...
function matvec(M::LowRankMatrix, x)
	...
	return sum([matvec(M.Ms[i], x) for i in 1:length(M.Ms)]) # Your code here
end
...
Because if $n \times n \ge k \times 2 * n$, then $k \le \frac{n}{2}$
"""
...
👉 Write a function `numerical_rank` that calculates the singular values of a matrix `A`, and returns how many of them are not approximately zero.
...
function numerical_rank(A::AbstractMatrix; tol=1e-5)
	...
	return sum([Int(i>=tol) for i in LinearAlgebra.svd(A).S])
end
...
numerical_rank(k_rank_ones(2, 1, 3))

# ╔═╡ c8829a12-917a-4ff4-87c9-fe2b25aaa99c
numerical_rank(k_rank_ones(2, 3, 1))
...
# ╔═╡ 45baf7a3-7bde-4e59-8382-b13879411120
let
	if !@isdefined(RankOneMatrix)
		not_defined(:RankOneMatrix)
	else
...
# ╔═╡ 750dd229-0cb7-4926-9745-1469beb07a66
let
	if !@isdefined(RankOneMatrix)
		not_defined(:RankOneMatrix)
	else

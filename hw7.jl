...
# ╔═╡ f1dd4abf-66d7-4fa4-bc39-cc251199f768
using Statistics
...
begin
    mutable struct Agent
        ...
        #  the number of other agents that this agent infects
        num_infected::Int64
    end
...
function interact!(agent::Agent, source::Agent, infection::InfectionRecovery)
    if is_susceptible(agent)
        if is_infected(source) && bernoulli(infection.p_infection)
            set_status!(agent, I)
...
# ╔═╡ 7299d07c-e9d7-42b6-b695-ae4bbc05e0f8
function counts(agents::Vector{Agent})
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
    ...
    ss, is, rs = zeros(T), zeros(T), zeros(T)
    ...
    plot(1:T, ss ./ length(simulations), label = "Susceptible")
    plot!(1:T, is ./ length(simulations), label = "Infectious")
    plot!(1:T, rs ./ length(simulations), label = "Recovered")
...
@bind p_infection Slider(0:0.01:1, default = 0.05, show_value = true)

# ╔═╡ 82c855af-594d-4891-82be-43903ac5644b
@bind p_recovery Slider(0:0.01:1, default = 0.01, show_value = true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
    ...
    ss, is, rs = map(x -> x ./ length(simulations), [ss, is, rs])
    sσ, iσ, rσ = map(Statistics.std, [ss, is, rs])

    plot(1:T, is, yerr = iσ, markerstrokecolor = :auto, label = "Infectious")
...
md"""
1. the maximum num of infected(recovered exclued) during a peroid of time;
2. the maximum slope/gradient of infection;
3. the num_infected distribution of each agent.
"""
...
let
    ...
    for sim in simulations
        plot!(p, 1:1000, sim.I, alpha = 0.5, label = nothing)
    end
...
# ╔═╡ 188a703b-6c73-47da-ad89-bd7a77a51156
let
    simulations =
        repeat_simulations(10000, 1000, InfectionRecovery(p_infection, p_recovery), 20)
    sir_mean_plot(simulations)
end

# ╔═╡ 34f93531-ef85-42d8-9222-987a8c49cc95
let
    simulations =
        repeat_simulations(1000, 1000, InfectionRecovery(p_infection, p_recovery), 50)
...
let
    ...
    histogram([agent.num_infected for agent in agents])
end
...
if !@isdefined(generate_agents)
    ...
    let
        ...
        elseif result isa Nothing
            keep_working(
                "The function returned `nothing`. Did you forget to return something?",
            )
        elseif !(result isa Vector) || !all(x -> x isa Agent, result)
            keep_working(
                md"Make sure that you return an array of objects of the type `Agent`.",
            )
        elseif length(result) != 4
            almost(md"Make sure that you return `N` agents.")
        elseif length(Set(result)) != 4
            almost(
                md"You returned the **same** agent `N` times. You need to call the `Agent` constructor `N` times, not once.",
            )
        else
    ...
    let
        ...
        elseif agent.status != S
            ...
            if source.status != S || source.num_infected != 0 || agent.status != S
                keep_working(
                    md"The `agent` should get infected with the right probability if the source is infectious.",
                )
            else
                agent = Agent(S, 9)
                source = Agent(I, 3)
                ...
                if agent.status == R
                    almost(
                        md"The agent should not recover immediately after becoming infectious.",
                    )
                elseif agent.status == S
                    keep_working(
                        md"The `agent` should recover from an infectious state with the right probability.",
                    )
                elseif source.status != I || source.num_infected != 4
                    almost(
                        md"The `source` did not get updated correctly after infecting the `agent`.",
                    )
                else
                    correct(
                        md"Your function treats the **susceptible** agent case correctly!",
                    )
                end
            end
        end
...
if !@isdefined(interact!)
    ...
    let
        ...
        elseif agent.num_infected != 9
            keep_working(
                md"`agent.num_infected` should not be modified if `agent` is infectious.",
            )
        else
            let
                agent = Agent(I, 9)
                ...
                if agent.status == R
                    ...
                    correct(
                        md"Your function treats the **infectious** agent case correctly!",
                    )
                end
            end
        end
...
if !@isdefined(interact!)
    ...
    let
        ...
        elseif agent.status != R || agent.num_infected != 9
            keep_working(
                md"The `agent` should not be momdified if it is in a recoved state.",
            )
        else
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "1b11da493a18a0d6083c7422d36049f8dd4461d2"

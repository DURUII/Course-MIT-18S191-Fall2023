...
Text(rand(Char, 5))   # sample 5 random characters

# ╔═╡ b61722cc-f98f-11ea-22ae-d755f61f78c3
Text(String(rand(Char, 40)))   # join into a string
...
# ╔═╡ 33c768cf-e860-4c25-ab6e-fb1370c8cb42
function not_occur()
	unused_letters = []
	for letter in alphabet
		if sample_freqs[index_of_letter(letter)] < 1e-6
			push!(unused_letters, letter)
		end
	end
...
unused_letters = not_occur() # replace with your answer
...
begin 
	...
	for letter in alphabet
		...
		if sample_freq_matrix[index, index] > 1e-6
			push!(double_letters, letter)
		end
	end
	
	double_letters # replace with your answer
end
...
most_likely_to_follow_w = alphabet[
	findmax(sample_freq_matrix[index_of_letter('w'), :])[2]
...
most_likely_to_precede_w = alphabet[
	findmax(sample_freq_matrix[:, index_of_letter('w')])[2]
] # replace with your answer
...
# ╔═╡ a8adb08b-079d-4678-b3a3-205179a3077b
trigrams = ngrams(split("to be or not to be that is the question", " "), 3)
...
function completion_cache(grams)
	...
	for item in grams
		if haskey(cache, item[1:length(item)-1])
			push!(cache[item[1:length(item)-1]], item[length(item)])
		else
			cache[item[1:length(item)-1]] = [item[length(item)]]
		end
	end
